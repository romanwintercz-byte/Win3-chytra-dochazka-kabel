...
const TimesheetTable: React.FC<TimesheetTableProps> = ({ entries, onDelete, onEdit, isLocked, jobs }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-20 md:mb-8 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left min-w-[500px]">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="px-3 py-4 sm:p-4 font-bold text-gray-500 uppercase text-[10px]">Datum</th>
              <th className="px-3 py-4 sm:p-4 font-bold text-gray-500 uppercase text-[10px]">Projekt & Typ</th>
              <th className="px-3 py-4 sm:p-4 font-bold text-gray-500 uppercase text-[10px]">Popis</th>
              <th className="px-3 py-4 sm:p-4 font-bold text-gray-500 uppercase text-[10px] text-right">Hodiny</th>
              {!isLocked && <th className="px-3 py-4 sm:p-4"></th>}
            </tr>
          </thead>
            ...
            {sorted.map((e, idx) => {
              ...
              return (
                <tr key={e.id} className={`hover:bg-slate-50 transition-colors ${!isFirstInDay ? 'bg-slate-50/40 border-l-4 border-l-indigo-300' : ''}`}>
                  <td className="px-3 py-4 sm:p-4 align-top">
                    {isFirstInDay ? (
                      <div>
                      ...
                      <div className="text-[9px] text-slate-300 italic pl-2 sm:pl-4">pokračování...</div>
                    )}
                  </td>
                  ...
                  <td className="px-3 py-4 sm:p-4 text-right font-black text-slate-900">{e.hours.toFixed(1)}h</td>
                  {!isLocked && (
                    <td className="px-3 py-4 sm:p-4 text-right whitespace-nowrap">
