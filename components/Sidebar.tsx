    ...
    <div className="hidden md:flex flex-col w-64 bg-slate-900 text-white min-h-screen sticky top-0 h-screen z-30">
      ...
      <div className="p-6 border-b border-slate-700">
        ...
        <div className="flex flex-col">
            <span className="text-[10px] text-slate-400 font-medium">by Win3 Studio</span>
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        >
          ...
          <span className="font-medium text-sm">Přehled & Zadání</span>
        </button>
        ...
        >
          ...
          <span className="font-medium text-sm">Reporty</span>
        </button>
        ...
        {currentUser.role === 'Manager' && (
          ...
          >
            ...
            <span className="font-medium text-sm">Admin & Nastavení</span>
          </button>
        )}
        ...
        {currentUser.role !== 'Manager' && (
            ...
            >
                ...
                <span className="font-medium text-sm">Napsat vedení</span>
            </button>
        )}
      </nav>

      {/* Spodní část s uživatelem a instalací */}
      <div className="p-4 border-t border-slate-700 space-y-4">
        {installPrompt && (
          <button
            onClick={onInstall}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-all text-xs font-bold shadow-lg shadow-indigo-900/20"
          >
        ...
        <div className="bg-slate-800 p-3 rounded-xl border border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Přihlášen</span>
            {onlineUserIds?.has(currentUser.id) && (
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                <span className="text-[8px] text-green-500 font-bold">ONLINE</span>
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-slate-400 font-medium uppercase">{currentUser.role}</p>
            </div>
          </div>
          ...
          <select 
            ...
            className="w-full text-xs bg-slate-900 border border-slate-600 text-slate-300 rounded-lg px-2 py-2 focus:outline-none focus:border-indigo-500 cursor-pointer hover:bg-black transition-colors"
          >
            {employees.map(emp => (
